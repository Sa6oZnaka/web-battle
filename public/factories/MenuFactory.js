...
export class MenuFactory {
    ...
    static fieldMenu(name, buildings) {
        ...
        for (let i = 0; i < 6; i++) {
            if (buildings[i] !== undefined) {
                components.set("button" + i, (new ImageButton(160 * (i + 1), 90 + 180, 155, 200, buildings[i].name, buildings[i].level)));
            } else {
    ...
    static newBuildingMenu(name, buildings) {

        buildings = [];
        buildings.push(BuildingFactory.mine());
    ...
    static buildingMenu(name, buildings) {
        ...
        for (let i = 0; i < 6; i++) {
            if (buildings.resources[i] !== undefined) {
                components.set("button" + i, (new ImageButton(160 * (i + 1), 90 + 180, 155, 200, buildings.resources[i].name, buildings.resources[i].amount)));
